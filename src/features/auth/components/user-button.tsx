...
export const UserButton = () => {
  ...
  const avatarFallbackValue = name ? name.charAt(0).toLocaleUpperCase() : email.charAt(0).toLocaleUpperCase() ?? "U";

  return (
    <DropdownMenu modal={false}>
      <DropdownMenuTrigger className="outline-none relative">
        <Avatar className="size-10 hover:opacity-75 transition border border-neutral-200">
          <AvatarFallback className=" bg-neutral-200  hover:text-amber-700 font-medium text-neutral-500 flex items-center justify-center">
            {avatarFallbackValue}
          </AvatarFallback>
        </Avatar>
      </DropdownMenuTrigger>
