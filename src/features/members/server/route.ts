...
const app = new Hono()
  ...
  .delete("/:memberId", sessionMiddleware, async (c) => {
    ...
    //If someone  is not the admin  and wants to delete other members  => Unauthorized
    // anyone is  just authorized to delete its membership from a workspace
    if (member.$id !== memberToDelete.$id && member.role !== MemberRole.ADMIN) {
      return c.json({ error: "Unauthorized" }, 401);
    }

    if (allMembersInWorkspace.total === 1) {
      return c.json({ error: "Can not delete the only member" }, 401);
    }
  ...
  .patch(
    ...
    async (c) => {
      ...
      if (allMembersInWorkspace.total === 1) {
        return c.json({ error: "Can not downgrade the only member" }, 401);
      }
